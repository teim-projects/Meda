...
class MedaParser:
    ...
    def parse_and_store_records(cls, month_str, raw_data_list, request_log=None):
        ...
        for item in raw_data_list:
            ...
            try:
                # 2. Extract fields using key fallbacks (camelCase and snake_case)
                ...
                generator_name = item.get('generatorName') or item.get('generator_name') or ''
                ...
                agreement_type_description = item.get('agreementTypeDescription') or item.get('agreement_type_description') or ''
                ...
                # 3. Upsert using update_or_create to prevent duplicate imports
                EnergyCreditNote.objects.update_or_create(

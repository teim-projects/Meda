...
import { RefreshCw, Droplets, Zap, Award, BarChart2 } from 'lucide-react';
...
const DEFAULT_DISTRICTS = [
  { district: 'Pune', count: 12, capacity_mw: 80.70, percentage: 21.57, color: '#2563eb' },
  { district: 'Kolhapur', count: 14, capacity_mw: 53.60, percentage: 14.33, color: '#1e3a8a' },
  { district: 'Satara', count: 9, capacity_mw: 28.48, percentage: 7.61, color: '#ea580c' },
  { district: 'Bhandara', count: 2, capacity_mw: 26.50, percentage: 7.08, color: '#4f46e5' },
  { district: 'Thane', count: 3, capacity_mw: 25.50, percentage: 6.82, color: '#059669' },
  { district: 'Ahilyanagar', count: 4, capacity_mw: 23.00, percentage: 6.15, color: '#7c3aed' },
  { district: 'Parbhani', count: 2, capacity_mw: 22.50, percentage: 6.01, color: '#0d9488' },
  { district: 'Sangli', count: 4, capacity_mw: 20.00, percentage: 5.35, color: '#e11d48' },
  { district: 'Chhatrapati Sambhajinagar', count: 2, capacity_mw: 12.00, percentage: 3.21, color: '#d97706' },
  { district: 'Solapur', count: 2, capacity_mw: 12.00, percentage: 3.21, color: '#0891b2' },
  { district: 'Sindhudurg', count: 3, capacity_mw: 11.70, percentage: 3.13, color: '#16a34a' },
  { district: 'Nashik', count: 3, capacity_mw: 10.00, percentage: 2.67, color: '#c2410c' },
  { district: 'Palghar', count: 2, capacity_mw: 6.75, percentage: 1.80, color: '#9333ea' },
  { district: 'Raigad', count: 2, capacity_mw: 4.00, percentage: 1.07, color: '#3b82f6' }
];

const DEFAULT_TIMELINE = [
  { year: 2000, cumulative_mw: 135.0 },
  { year: 2004, cumulative_mw: 160.0 },
  { year: 2006, cumulative_mw: 175.0 },
  { year: 2008, cumulative_mw: 215.0 },
  { year: 2010, cumulative_mw: 275.0 },
  { year: 2012, cumulative_mw: 310.0 },
  { year: 2014, cumulative_mw: 335.0 },
  { year: 2016, cumulative_mw: 374.08 }
];

export const SmallHydroDashboard = () => {
  const [loading, setLoading] = useState(true);
  const [totalProjects, setTotalProjects] = useState(70);
  const [totalCapacityMw, setTotalCapacityMw] = useState(374.08);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.warn('Small hydro analytics fallback:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full bg-[#f4f7fb] p-2 sm:p-4 md:p-6 font-sans text-slate-800 antialiased space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-[760px]">
        <div className="bg-[#0b1b3d] rounded-xl p-5 text-white shadow-sm flex flex-col justify-between min-h-[105px] border border-slate-900/80">
          <div className="text-3xl lg:text-4xl font-bold tracking-tight text-white leading-none">
            {Number(totalCapacityMw).toFixed(2).replace(/\.00$/, '')}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-700/80 p-5 shadow-sm flex flex-col justify-between relative">
            ...
            {/* Plot area (Ticks: 1, 10 Log Scale matching Screenshot 2) */}
            <div className="relative flex items-stretch h-[240px] pt-4">
              ...
              <div className="w-8 shrink-0 flex flex-col justify-between py-1 text-right pr-2 text-[10px] font-medium text-slate-500 select-none">
                <span className="relative -top-2">10</span>
              ...
              <div className="flex-1 relative border-l border-b border-slate-300 overflow-x-auto overflow-y-hidden pb-1">
                ...
                <div className="h-full flex items-end min-w-max px-2 gap-[6px] pt-2">
                  {districts.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="flex flex-col items-center justify-end h-full w-[19px] group cursor-pointer relative"
            ...
            <div className="flex items-start ml-[45px] overflow-x-auto min-h-[85px] pt-1">
              <div className="flex items-start min-w-max px-2 gap-[6px]">
                {districts.map((item, idx) => {
                  const isSelected = activeBox.district === item.district;
                  return (
                    <div
                      key={idx}
                      className="w-[19px] flex justify-center cursor-pointer"
                      onMouseEnter={() => setInspectedDistrict({ ...item, rank: idx + 1 })}
                      onClick={() => setInspectedDistrict({ ...item, rank: idx + 1 })}
                    >
                      <span
          ...
          <div className="bg-white rounded-2xl border border-slate-700/80 p-5 shadow-sm">
            ...
            <div className="relative w-full h-[140px]">
              <svg viewBox="0 0 500 130" className="w-full h-full overflow-visible">
                ...
                <text x="28" y="29" fontSize="9" fill="#94a3b8" textAnchor="end">200</text>
                <text x="28" y="113" fontSize="9" fill="#94a3b8" textAnchor="end">0</text>

                <defs>
                  <linearGradient id="shpGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#0284c7" stopOpacity="0.25" />
                    <stop offset="100%" stopColor="#0284c7" stopOpacity="0.0" />
                  </linearGradient>
                </defs>
                <path d={timelineAreaD} fill="url(#shpGrad)" />
        ...
        <div className="lg:col-span-5 bg-white rounded-2xl border border-slate-700/80 p-5 shadow-sm flex flex-col justify-between">
          ...
          <div className="text-center mb-1">
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">
              Small Hydro Power Distribution
            </p>
          </div>
      ...
      {/* 3. AESTHETIC BOTTOM GRAPH: REGIONAL HYDROELECTRIC HUBS */}
      <div className="bg-white rounded-2xl border border-slate-700/80 p-6 shadow-sm">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-slate-900 tracking-tight">
                Small Hydro River Basins & Western Ghats Distribution
              </h2>
            </div>
            <p className="text-xs text-slate-500 mt-1">
              Run-of-the-river and canal drop hydro electric installations across Maharashtra
            </p>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5">
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80">
            <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pune & Sahyadri Foothills</div>
            <div className="text-xl font-extrabold text-slate-900 mt-1">Pune & Kolhapur</div>
            <div className="text-sm font-bold text-blue-700 mt-0.5">134.30 MW • 26 Projects</div>
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80">
            <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Krishna & Bhima Basins</div>
            <div className="text-xl font-extrabold text-slate-900 mt-1">Satara, Sangli, Solapur</div>
            <div className="text-sm font-bold text-emerald-700 mt-0.5">60.48 MW • 15 Projects</div>
            <div className="w-full bg-slate-200 rounded-full h-1.5 mt-3">
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80">
            <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Konkan Coastal & Vidarbha</div>
            <div className="text-xl font-extrabold text-slate-900 mt-1">Thane, Bhandara, Sindhudurg</div>
            <div className="text-sm font-bold text-amber-700 mt-0.5">63.70 MW • 8 Projects</div>
            <div className="w-full bg-slate-200 rounded-full h-1.5 mt-3">
              <div className="bg-amber-600 h-1.5 rounded-full" style={{ width: '75%' }} />
            </div>
          </div>
        </div>
      </div>
...
export default SmallHydroDashboard;

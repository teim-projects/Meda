...
const DEFAULT_DISTRICTS = [
  { district: 'Chandrapur', count: 3, capacity_mw: 33.0, percentage: 15.35, color: '#2563eb' },
  { district: 'Ahilyanagar', count: 1, capacity_mw: 25.0, percentage: 11.63, color: '#1e3a8a' },
  { district: 'Gadchiroli', count: 2, capacity_mw: 20.0, percentage: 9.30, color: '#ea580c' },
  { district: 'Nagpur', count: 2, capacity_mw: 20.0, percentage: 9.30, color: '#4f46e5' },
  { district: 'Nanded', count: 1, capacity_mw: 20.0, percentage: 9.30, color: '#059669' },
  { district: 'Chhatrapati Sambhajinagar', count: 1, capacity_mw: 13.0, percentage: 6.05, color: '#7c3aed' },
  { district: 'Bhandara', count: 1, capacity_mw: 10.0, percentage: 4.65, color: '#0d9488' },
  { district: 'Hingoli', count: 1, capacity_mw: 10.0, percentage: 4.65, color: '#e11d48' },
  { district: 'Jalgaon', count: 1, capacity_mw: 10.0, percentage: 4.65, color: '#d97706' },
  { district: 'Parbhani', count: 1, capacity_mw: 10.0, percentage: 4.65, color: '#0891b2' },
  { district: 'Sangli', count: 1, capacity_mw: 10.0, percentage: 4.65, color: '#16a34a' },
  { district: 'Wardha', count: 1, capacity_mw: 10.0, percentage: 4.65, color: '#c2410c' },
  { district: 'Yeotmal', count: 1, capacity_mw: 10.0, percentage: 4.65, color: '#9333ea' },
  { district: 'Gondia', count: 1, capacity_mw: 10.0, percentage: 4.65, color: '#3b82f6' },
  { district: 'Nashik', count: 1, capacity_mw: 8.0, percentage: 3.72, color: '#64748b' }
];

const DEFAULT_TIMELINE = [
  { year: 2005, cumulative_mw: 8.0 },
  { year: 2006, cumulative_mw: 18.0 },
  { year: 2008, cumulative_mw: 65.0 },
  { year: 2009, cumulative_mw: 95.0 },
  { year: 2010, cumulative_mw: 145.0 },
  { year: 2012, cumulative_mw: 160.0 },
  { year: 2013, cumulative_mw: 170.0 },
  { year: 2014, cumulative_mw: 180.0 },
  { year: 2015, cumulative_mw: 190.0 },
  { year: 2016, cumulative_mw: 215.0 }
];

export const BiomassDashboard = () => {
  const [loading, setLoading] = useState(true);
  const [totalProjects, setTotalProjects] = useState(19);
  const [totalCapacityMw, setTotalCapacityMw] = useState(215.00);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.warn('Biomass analytics fallback:', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-700/80 p-5 shadow-sm flex flex-col justify-between relative">
            ...
            {/* Plot area (Ticks: 0, 10, 20, 30 matching Screenshot 2) */}
          ...
          {/* TIMELINE GRAPH: Capacity Installed (MW) over years (2006 to 2016) */}
          <div className="bg-white rounded-2xl border border-slate-700/80 p-5 shadow-sm">
            ...
            <div className="relative w-full h-[140px]">
              <svg viewBox="0 0 500 130" className="w-full h-full overflow-visible">
                ...
                {timeline
                  .filter((_, idx) => idx % Math.ceil(timeline.length / 6) === 0 || idx === timeline.length - 1)
                  .map((pt, idx) => {
      ...
      <div className="bg-white rounded-2xl border border-slate-700/80 p-6 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Agricultural residue energy plants across Eastern & Central Maharashtra
            </p>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5">
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80">
            <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Eastern Vidarbha Hub</div>
            <div className="text-xl font-extrabold text-slate-900 mt-1">Chandrapur & Gadchiroli</div>
            <div className="text-sm font-bold text-blue-700 mt-0.5">53.00 MW • 5 Projects</div>
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80">
            <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Nagpur & Wardha Cluster</div>
            <div className="text-xl font-extrabold text-slate-900 mt-1">Nagpur, Wardha, Bhandara</div>
            <div className="text-sm font-bold text-emerald-700 mt-0.5">40.00 MW • 4 Projects</div>
            <div className="w-full bg-slate-200 rounded-full h-1.5 mt-3">
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80">
            <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Marathwada & Western Hub</div>
            <div className="text-xl font-extrabold text-slate-900 mt-1">Ahilyanagar, Nanded, Chhatrapati Sambhajinagar</div>
            <div className="text-sm font-bold text-amber-700 mt-0.5">58.00 MW • 3 Projects</div>
            <div className="w-full bg-slate-200 rounded-full h-1.5 mt-3">
              <div className="bg-amber-600 h-1.5 rounded-full" style={{ width: '82%' }} />
            </div>
          </div>
        </div>
      </div>

...
import { RefreshCw, Wind, Zap, Award, BarChart2 } from 'lucide-react';
import { energyApi } from '../../services/energyApi';

const PALETTE = [
  '#2563eb', '#1e3a8a', '#ea580c', '#4f46e5', '#059669',
  '#d97706', '#0284c7', '#7c3aed', '#dc2626', '#0891b2',
  '#65a30d', '#9333ea', '#c026d3', '#0d9488', '#e11d48',
  '#3b82f6', '#10b981'
];
...
export const WindDashboard = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        setTotalProjects(res.total_projects || 3063);
        setTotalCapacityMw(res.total_capacity_mw || 6371.81);
  ...
  const getLogBarHeightPct = (mw) => {
    ...
    const maxLog = 3.30; // slightly above log10(1637.5) = 3.214
    const pct = (logVal / maxLog) * 100;
    return Math.min(Math.max(pct, 6), 98);
  };

  // Simplified pie chart segments (Top 6 + Others matching Bagasse exactly)
  ...
  const currentCenterDisplay = activePieHover || {
    ...
    mw: `${Number(totalCapacityMw).toLocaleString('en-IN', { maximumFractionDigits: 1 })} MW`,
    sub: `${totalProjects.toLocaleString()} Projects`
  };

  // SVG Timeline points calculation (exact match to Bagasse: 500x130, margin 40/110)
  ...
  return (
    <div className="w-full bg-[#f4f7fb] p-2 sm:p-4 md:p-6 font-sans text-slate-800 antialiased space-y-6">
      ...
      {/* 1. TOP KPI METRIC CARDS (Exact Match to Bagasse: max-w-[760px], min-h-[105px]) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-[760px]">
        <div className="bg-[#0b1b3d] rounded-xl p-5 text-white shadow-sm flex flex-col justify-between min-h-[105px] border border-slate-900/80">
          <div className="text-3xl lg:text-4xl font-bold tracking-tight text-white leading-none">
            {Number(totalCapacityMw).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </div>
      ...
      {/* 2. MIDDLE SECTION: BAR CHART + TIMELINE (LEFT 7 COLS) & PIE CHART (RIGHT 5 COLS) */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Wind Turbine Power Distribution by District
                </p>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200/90 rounded-xl px-3.5 py-2 min-w-[210px] shadow-sm flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[11px] text-slate-600 flex items-center justify-between gap-3 mt-0.5">
                    <span><strong>{activeBox.count}</strong> turbines</span>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-slate-100">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-slate-900 tracking-tight">
                Maharashtra Major Wind Power Corridors
              </h2>
            </div>
          ...
          <div className="flex items-center gap-2">
            <div className="bg-blue-50 text-blue-800 px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5">
              <Zap size={14} />
              <span>6,371.81 MW Commissioned</span>
            </div>
          </div>
        </div>
